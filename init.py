...
if os.path.exists("Log_file.txt"):
    with open("Log_file.txt", "r+") as f_d:
        f_d.truncate(0)
...
logging.basicConfig(
    ...
    handlers=[
        RotatingFileHandler(
            "Log_file.txt", maxBytes=50000000, backupCount=10
        ),
...
#    os.environ.get("MAX_TIME_TO_WAIT_FOR_TORRENTS_TO_START", 600)
    os.environ.get("MAX_TIME_TO_WAIT_FOR_TORRENTS_TO_START", 86400)
)
...
CANCELALL_COMMAND_G = os.environ.get("CANCELALL_COMMAND_G", "cancelall")
...
USAGE_COMMAND = os.environ.get("USAGE_COMMAND", "usage")
